...
dict 字段，就像编辑 JSON 一样。PB 和 JSON 在操作层完全一致。
...
| addon: OK, traffic: NO traffic | addon 已启动但没有流量 | 代理/证书未就绪 或 应用空闲。让用户操作应用（打开页面/搜索）触发流量，然后重新检查。如果应用有活动后流量仍为 0 → 证书未安装或设备代理未设置（指引：设备访问 http://mitm.it 安装 CA；`adb reverse tcp:8080 tcp:8080` + `adb shell settings put global http_proxy 127.0.0.1:8080`）。 |
