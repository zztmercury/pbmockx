...
## 启动
```bash
pbmockx start             # 起 mitmdump + addon + adb reverse + 设备代理
```
...
## 文件
- `addon/pbmockx_addon.py` — mitmproxy addon（协议识别/PB/JSON/dict/双向/mock/control API/contentview）
- `bin/pbmockx` — CLI（纯 stdlib，调 control API + 工具维护）
- `scripts/install.sh` — 一键安装（curl | sh）
- `scripts/start.sh` — 启动脚本（adb reverse + 设备代理 + mitmdump）
